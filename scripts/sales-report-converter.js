...
function parseCSV(csvContent) {
	...
	return lines.slice(1).map((line) => {
		...
		headers.forEach((header, index) => {
			...
			// Normalize whitespace-only cells to empty
...
function generateChartConfigurations(salesData) {
	...
	Object.entries(dataByCategory).forEach(([category, data]) => {
		if (category === 'brand_performance') {
			// Market Share by Region (macro regions only)
			const macroRegions = [
				'Nation',
				'Northeast',
				'Southeast',
			...
			const macroRows = macroRegions
				.map((mr) =>
					data.find(
						(row) => row.segment === 'All IDN' && row.region === mr
					)
				)
				.filter(Boolean);
			...
			// Map chart generation moved to a post-processing step using full CSV
	...
	// Additional charts driven from CSV
	const hcp = salesData.filter((r) => r.category === 'hcp_attitudes');
	const political = salesData.filter(
		(r) => r.category === 'political_sentiments'
	);
	const series = salesData.filter((r) => r.category === 'series_completion');
	const operational = salesData.filter(
		(r) => r.category === 'operational_metrics'
	);

	const prr = hcp
		.filter((r) => r.region && r.recall_rate != null)
		.map((r) => ({ region: r.region, recall_rate: Number(r.recall_rate) }));
	if (prr.length > 0) {
	...
	const wvg = hcp
		.filter((r) => r.period && r.growth_rate != null)
		.map((r) => ({ period: r.period, growth_rate: Number(r.growth_rate) }));
	if (wvg.length > 0) {
	...
	const wvag = hcp
		.filter((r) => r.age_group && r.visit_count != null)
		.map((r) => ({
	...
	const br = political
		.filter((r) => r.year && r.birth_rate != null)
		.map((r) => ({
			year: String(r.year),
			birth_rate: Number(r.birth_rate),
		}));
	if (br.length > 0) {
		charts.birth_rate = {
			id: 'birth_rate',
	...
	const vrsc = political
		.filter((r) => r.state && r.vaccination_rate != null)
		.map((r) => ({
			state: r.state,
			vaccination_rate: Number(r.vaccination_rate),
		}));
	if (vrsc.length > 0) {
	...
	const scr = series
		.filter((r) => r.region && r.completion_rate != null)
		.map((r) => ({
			region: r.region,
	...
	const md = series
		.filter((r) => r.month && r.missed_count != null)
		.map((r) => ({ month: r.month, missed_count: Number(r.missed_count) }));
	if (md.length > 0) {
		charts.missed_doses = {
			id: 'missed_doses',
	...
	const ppm = series
		.filter((r) => r.age_group && r.population_change != null)
		.map((r) => ({
			age_group: r.age_group,
			population_change: Number(r.population_change),
		}));
	if (ppm.length > 0) {
	...
	const cav = operational
		.filter((r) => r.territory && r.call_count != null)
		.map((r) => ({
	...
	const tp = operational
		.filter((r) => r.month && r.performance_score != null)
		.map((r) => ({
			month: r.month,
	...
	const asg = operational
		.filter((r) => r.account_type && r.count != null)
		.map((r) => ({ account_type: r.account_type, count: Number(r.count) }));
	if (asg.length > 0) {
	...
	const pla = operational
		.filter((r) => r.product && r.revenue != null)
		.map((r) => ({ product: r.product, revenue: Number(r.revenue) }));
	if (pla.length > 0) {
 ...
 */
function generatePageLayouts(chartConfigurations) {
	const allCharts = Object.values(chartConfigurations || {});

	// Stable priority for chart ordering
	const chartPriority = [
	...
	const priorityIndex = (id) => {
		const idx = chartPriority.indexOf(id);
		return idx === -1 ? Number.MAX_SAFE_INTEGER : idx;
	};

	// Sort charts by priority for stable ordering
	allCharts.sort((a, b) => priorityIndex(a.id) - priorityIndex(b.id));

	const filterExisting = (ordered) =>
		ordered.filter((id) => id in chartConfigurations);

	const brandCharts = filterExisting([
		'market_share_regions',
		'market_share_change_segments',
		'revenue_by_region_map',
		'growth_trend_regions',
	...
	const forecastCharts = filterExisting([
		'revenue_vs_forecast',
		'revenue_components',
		'forecast_trend',
	...
	const hcpCharts = filterExisting([
		'postcard_recall_rate',
		'wellness_visit_growth',
		'wellness_visit_by_age_group',
	]);

	const politicalCharts = filterExisting([
		'birth_rate',
		'vaccination_rate_south_central',
	]);

	const seriesCharts = filterExisting([
		'series_completion',
		'missed_doses',
		'ped_population_movement',
	]);

	const opsCharts = filterExisting([
		'call_activity_volume',
		'territory_performance',
		'account_segmentation',
		'product_line_analysis',
	...
	].filter((c) => c.chart in chartConfigurations);
...
function convertSalesReportToJSON() {
	try {
		...
		(function ensureMapFromCsv() {
			...
			let stateRows = salesData.filter(
				(row) => row.state && (row.revenue != null || row.value != null)
			);
			const brandState = stateRows.filter(
				(r) =>
					r.category === 'brand_performance' &&
					r.segment === 'All IDN'
			);
			if (brandState.length > 0) stateRows = brandState;

			let stateData = stateRows.map((row) => ({
				name: row.state,
				value: Number(row.revenue != null ? row.revenue : row.value),
			}));
			...
			if (stateData.length > 0) {
				...
				chartConfigurations.revenue_by_region_map = {
		...
		// Ensure legacy charts exist even if CSV lacks rows
		(function ensureLegacyCharts() {
			// Helper: add chart only if missing
			const addIfMissing = (id, chart) => {
				if (!chartConfigurations[id]) chartConfigurations[id] = chart;
			};

			// vaxneuvance_share_regions: reuse market_share by region if present, else fallback fixed
			if (!chartConfigurations.vaxneuvance_share_regions) {
				const brandAllIdn = salesData.filter(
					(r) =>
						r.category === 'brand_performance' &&
						r.segment === 'All IDN'
				);
				const data = brandAllIdn.length
					? brandAllIdn.map((r) => ({
							region: r.region,
							market_share: Number(r.market_share || 0),
						}))
					: [
							{ region: 'Nation', market_share: 94 },
							{ region: 'Northeast', market_share: 96 },
							{ region: 'Southeast', market_share: 91 },
							{ region: 'West', market_share: 90 },
							{ region: 'South Central', market_share: 68 },
							{ region: 'Greater', market_share: 81 },
						];
				addIfMissing('vaxneuvance_share_regions', {
					id: 'vaxneuvance_share_regions',
			...
			// qoq_market_share_change: by region from change_percentage; fallback fixed
			if (!chartConfigurations.qoq_market_share_change) {
				const brandAllIdn = salesData.filter(
					(r) =>
						r.category === 'brand_performance' &&
						r.segment === 'All IDN'
				...
				const data = brandAllIdn.length
					? brandAllIdn.map((r) => ({
							region: r.region,
							change_percentage: Number(r.change_percentage || 0),
						}))
					: [
							{ region: 'Nation', change_percentage: 24.2 },
							{ region: 'Northeast', change_percentage: 18.0 },
							{ region: 'Southeast', change_percentage: 12.0 },
							{ region: 'West', change_percentage: 10.0 },
							{ region: 'South Central', change_percentage: 6.0 },
							{ region: 'Greater', change_percentage: 8.0 },
						];
				addIfMissing('qoq_market_share_change', {
					id: 'qoq_market_share_change',
			...
			// le_revenue_change_baseline: compute from revenue_forecast or fallback 0s
			if (!chartConfigurations.le_revenue_change_baseline) {
				const nat = salesData.filter(
					(r) =>
						r.category === 'revenue_forecast' &&
						r.region === 'National'
				);
				const baseline = nat.find(
					(r) => String(r.segment).toLowerCase() === 'baseline'
				);
				const current = nat.find(
					(r) => String(r.segment).toLowerCase() === 'current'
				);
				const value =
					baseline && current
						? (Number(current.value || current.revenue || 0) -
								Number(
									baseline.value || baseline.revenue || 0
								)) /
							1000000
						: 0;
				addIfMissing('le_revenue_change_baseline', {
					id: 'le_revenue_change_baseline',
			...
			// HCP attitudes fallbacks
			addIfMissing('postcard_recall_rate', {
				id: 'postcard_recall_rate',
				title: 'Postcard recall rate',
				type: 'bar',
				category: 'hcp_attitudes',
			...
			addIfMissing('wellness_visit_growth', {
				id: 'wellness_visit_growth',
				title: 'Wellness visit growth',
				type: 'line',
				category: 'hcp_attitudes',
			...
			addIfMissing('wellness_visit_by_age_group', {
				id: 'wellness_visit_by_age_group',
				title: 'Wellness visit by age group',
				type: 'bar',
				category: 'hcp_attitudes',
			...
			addIfMissing('vaccination_rate_south_central', {
				id: 'vaccination_rate_south_central',
				title: 'Vaccination rate in South Central',
				type: 'bar',
				category: 'political_sentiments',
			...
			// Series completion
			addIfMissing('series_completion', {
				id: 'series_completion',
				title: 'Series completion rate',
				type: 'bar',
				category: 'series_completion',
			...
			addIfMissing('ped_population_movement', {
				id: 'ped_population_movement',
				title: 'Pediatric population movement',
				type: 'bar',
				category: 'series_completion',
			...
			// Operational metrics
			addIfMissing('call_activity_volume', {
				id: 'call_activity_volume',
				title: 'Call activity volume',
				type: 'bar',
				category: 'operational_metrics',
			...
			addIfMissing('territory_performance', {
				id: 'territory_performance',
				title: 'Territory performance',
				type: 'line',
				category: 'operational_metrics',
			...
			addIfMissing('account_segmentation', {
				id: 'account_segmentation',
				title: 'Account segmentation',
				type: 'bar',
				category: 'operational_metrics',
			...
			addIfMissing('product_line_analysis', {
				id: 'product_line_analysis',
				title: 'Product line analysis',
				type: 'bar',
				category: 'operational_metrics',
